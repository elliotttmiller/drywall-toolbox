...
- `products/launch/official/` — exactly the two current production/runtime import CSV authorities.
- `products/Production/catalogs/config/production_taxonomy_policy.json` and the current category map when present — machine-readable production catalog governance only.
- root `.htaccess` and `logos/` required by the production web-root contract.

## Product-data boundary

`products/launch/official/` is the production import authority for this package. The builder requires exactly two CSV files in that directory and copies those two files only.
...
- legacy/stage/intermediate catalog CSV generations outside `products/launch/official/`
