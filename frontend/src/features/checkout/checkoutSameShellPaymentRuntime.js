...
function paymentActionFromRoot(root) {
  ...
  const actions = Array.from(paymentStep.querySelectorAll('button, a'));
...
function handleSameShellPaymentRequest(event) {
  ...
  void requestSameShellPayment(root, action, { force: event?.detail?.force === true });
}
...
function observePreparedPayment() {
  ...
  const observer = new MutationObserver(() => syncPreparedPayment());
