 ...
 * Gateway owns embedded payment methods, Link, eligible express wallets,
...
final class DTB_OfficialStripeNativeCheckout {
	...
	public static function checkout_capabilities(): WP_REST_Response {
		...
		foreach ( self::payment_gateways() as $gateway ) {
			...
			if ( ! self::is_official_stripe_gateway_id( $id ) ) {
				continue;
			}
	...
	public static function mirror_verified_stripe_payment( $order_id ): void {
		...
		self::tag_order( $order, 'woocommerce_stripe_lifecycle' );
	...
	public static function admin_notices(): void {
		...
		if ( ! self::is_official_stripe_gateway_enabled() ) {
			echo '<div class="notice notice-warning"><p>'
				. esc_html__( 'Drywall Toolbox checkout is configured for WooCommerce Checkout + the official WooCommerce Stripe Payment Gateway. Enable and connect the official Stripe gateway before accepting live checkout payments.', 'drywall-toolbox' )
				. '</p></div>';
		}

		if ( self::is_gateway_enabled( 'woocommerce_payments' ) ) {
			echo '<div class="notice notice-warning"><p>'
				. esc_html__( 'Drywall Toolbox checkout should have one active storefront card/wallet authority. Disable WooPayments when the official WooCommerce Stripe gateway is the active payment provider.', 'drywall-toolbox' )
				. '</p></div>';
		}
	...
	private static function is_official_stripe_order( WC_Order $order ): bool {
		return self::is_official_stripe_gateway_id( sanitize_key( (string) $order->get_payment_method() ) );
	}
	...
	private static function is_official_stripe_gateway_enabled(): bool {
		foreach ( self::payment_gateways() as $gateway ) {
			$id = sanitize_key( (string) ( $gateway->id ?? '' ) );
			if ( self::is_official_stripe_gateway_id( $id ) && isset( $gateway->enabled ) && 'yes' === (string) $gateway->enabled ) {
				return true;
			}
		}
		return false;
	...
	private static function is_official_stripe_gateway_id( string $gateway_id ): bool {
		return self::STRIPE_GATEWAY_ID === $gateway_id || str_starts_with( $gateway_id, self::STRIPE_GATEWAY_ID . '_' );
	}
}
