...
(function () {
	...
	function runtimeRoot() {
		return document.querySelector('body.dtb-order-pay-runtime');
	}
	...
	function gatewayHasDetails(method) {
		...
		if (!box) return false;
		return Boolean(compactText(box) || box.querySelector('input, select, textarea, iframe, button, .StripeElement, .wcpay-upe-element, .wcpay-payment-element'));
	}
	...
	function syncPaymentLead(counts) {
		...
		if (copy) {
			copy.textContent = counts.express > 0
				? 'Wallets, card fields, and verification are provided by WooCommerce and WooPayments.'
	...
	function syncMethods() {
		...
		Array.prototype.slice.call(host.querySelectorAll('.wc_payment_method')).forEach(function (method) {
			...
			var hasDetail = gatewayHasDetails(method);
	...
	function syncPayButton() {
		...
		if (!placeOrder.querySelector('.dtb-op-cta-trust')) {
			placeOrder.append(el('div', 'dtb-op-cta-trust', 'Encrypted payment. Gateway confirmation completes the charge.'));
		}
	}
	...
	function enhanceNotices() {
		...
		if (signature && signature !== lastNoticeSignature) {
			...
			window.setTimeout(function () {
				if (notices[0] && notices[0].scrollIntoView) notices[0].scrollIntoView({ block: 'center', behavior: 'smooth' });
			}, 80);
		}
	}
	...
	function bind() {
		...
		document.addEventListener('submit', function (event) {
			if (event.target && event.target.matches('.dtb-op-card form#order_review')) setSubmitting(true);
		}, true);
		document.addEventListener('click', function (event) {
			if (event.target && event.target.closest && event.target.closest('.dtb-op-card #place_order')) {
